        ...
        switch (binanceLimitOrder.getType()) {
            case BID -> {
                switch (binanceLimitOrder.getStatus()) {
                    ...
                    case CANCELED, EXPIRED, REJECTED, REPLACED -> Util.credit(counter, bid, "Cancelled Binance Buy Limit order");
                }
            }
            case ASK -> {
                switch (binanceLimitOrder.getStatus()) {
                    ...
                    case FILLED, PARTIALLY_FILLED -> {
                        Util.credit(counter, cumulativeAmount.multiply(limitPrice).setScale(8, RoundingMode.HALF_UP), "Filled or Partially Filled Binance Sell Limit order");
                        Util.debit(commissionAsset, fee, "Fee for Filled or Partially Filled Binance Sell Limit order");
                    }
                    case CANCELED, EXPIRED, REJECTED, REPLACED -> Util.credit(base, ask, "Cancelled Binance Sell Limit order");
                }
            }
