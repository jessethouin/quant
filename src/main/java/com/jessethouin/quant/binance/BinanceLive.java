...
//        currencyPairs.forEach(BinanceLive::subscribeToCurrencyTrades);
                ...
                .subscribe(order -> {
                    ...
                    if (order instanceof LimitOrder) {
                        ...
                        switch (order.getStatus()) {
                            case NEW -> {
                                binanceLimitOrder = createBinanceLimitOrder(limitOrder);
                                BinanceTransactions.processBinanceLimitOrder(binanceLimitOrder);
                                Database.persistBinanceLimitOrder(binanceLimitOrder);
                            }
                            case FILLED -> {
                                binanceLimitOrder = Objects.requireNonNullElse(Database.getBinanceLimitOrder(limitOrder.getId()), createBinanceLimitOrder(limitOrder));
    ...
    private static BinanceLimitOrder createBinanceLimitOrder(LimitOrder limitOrder) {
    ...
    private static void subscribeToMarketFeed(CurrencyPair currencyPair, String subscriptionType) {
        ...
        Disposable subscribe = subscription.subscribe(t -> {
            ...
            orderHistoryLookup = new OrderHistoryLookup(binanceTradeHistory.getTradeId(), 0);
            ...
            LOG.info("{}/{} - {} : ma1 {} : ma2 {} : l {} : h {} : p {} : v {}", baseCurrency.getSymbol(), counterCurrency.getSymbol(), ref.count, ref.shortMAValue, ref.longMAValue, ref.c.getLow(), ref.c.getHigh(), ref.price, Util.getBalance(portfolio, baseCurrency, counterCurrency, ref.price).add(Util.getBalance(portfolio, counterCurrency)));
